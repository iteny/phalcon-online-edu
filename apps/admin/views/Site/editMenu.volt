...
<div id="frame-content">
    <form name="editMenu" method="post" class="ajax-form" action="/admin/site/addEditMenu" novalidate="novalidate">
        ...
        <input type="hidden" name="id" value="{{thismenu['id']}}">
            ...
            <table cellpadding="0" cellspacing="0" class="table_form" width="100%">
                ...
                <tr>
                    ...
                    <td>
                        <select name="pid" class="length_4">
                            <option value="0" {% if thismenu['pid'] == 0 %}selected{% endif %}>顶级菜单</option>
                            {% for menu in adminSelect %}
                            <option value="{{menu['id']}}" {% if menu['id'] == thismenu['pid'] %}selected{% endif %}><?php echo str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;',$menu['level']); ?><if condition='$vo["level"] gt 0'>├─</if>{{menu['html']}}{{menu['name']}}</option>
                            {% endfor %}
                    </td>
                </tr>
                <tr>
                    <td>菜单名称:</td>
                    <td><input type="text" class="input length_5" name="name" value="{{thismenu['name']}}" id="name"></td>
                </tr>
                <tr>
                    <td>控制器:</td>
                    <td><input type="text" class="input length_5" name="controller" id="controller" value="{{thismenu['controller']}}"></td>
                </tr>
                <tr>
                    <td>方法:</td>
                    <td><input type="text" class="input length_5" name="action" id="action" value="{{thismenu['action']}}"></td>
                </tr>
                <tr>
                    <td>菜单排序:</td>
                    <td><input type="text" class="input length_5" name="sort" id="sort" value="{{thismenu['sort']}}"></td>
                </tr>
                <tr>
                    <td>菜单图标：</td>
                    <td>
                        <input id="System_Menu_icons_input" name="icon" type="hidden" value="{{thismenu['icon']}}">
                        <strong id="System_Menu_icons" style="margin-right: 10px;"><i style='color: #666;font-size: 16px;' class='iconfont'>{{thismenu['icon']}}</i></span></strong>
                ...
                <tr>
                    ...
                    <td><select name="isshow">
                        <option value="1" {% if thismenu['isshow'] == 1 %}selected{% endif %}>显示</option>
                        <option value="0" {% if thismenu['isshow'] == 0 %}selected{% endif %}>不显示</option>
...
<script>
    ...
    var checkEditMname = '/admin/SiteCom/checkEditMname/?id='+{{thismenu['id']}};
</script>
